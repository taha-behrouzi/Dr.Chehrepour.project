...
export default async function PodcastDetailPage({
  ...
  const isLocked = podcast.isPremium && !isAuthenticated;
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-8">
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 sm:p-10 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          >
            {podcast.isPremium ? "ویژه VIP" : "رایگان"}
          </span>
        ...
        <p className="text-slate-600 leading-relaxed mb-8">{podcast.description}</p>
        ...
        {isLocked ? (
          <div className="bg-navy text-cream p-8 rounded-2xl border border-gold/30 shadow-2xl text-center max-w-md mx-auto">
            ...
            <p className="text-cream/70 text-sm mb-6">
              این پادکست جزو محتوای ویژه است. برای پخش پادکست لطفا وارد حساب کاربری خود شوید.
            </p>
            <Link
              href="/login"
      ...
      {/* Comments Section */}
      <CommentsSection targetId={podcast.id} />
    </div>
  );
}
